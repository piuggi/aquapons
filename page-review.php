...
<?php get_header(); ?>
	...
	<section class="main">
		...
		if($current_user->roles[0]=='instructor'){
			...
			$reviews = $wpdb->get_results( "SELECT * FROM aq_badge_submissions WHERE current_status = '".$review_type."' AND user_id IN ($ids) ORDER BY submission_timestamp LIMIT $current_result, 20" );
			$pending_reviews = $wpdb->query( "SELECT * FROM aq_badge_submissions WHERE current_status = '".$review_type."' AND user_id IN ($ids) ORDER BY submission_timestamp LIMIT $current_result, 20" );
			
		}else{
		
			$reviews = $wpdb->get_results("SELECT * FROM aq_badge_submissions WHERE current_status = '".$review_type."' ORDER BY submission_timestamp LIMIT $current_result, 20");
			$pending_reviews = $wpdb->query("SELECT * FROM aq_badge_submissions WHERE current_status = '".$review_type."' ORDER BY submission_timestamp"); 
		}
